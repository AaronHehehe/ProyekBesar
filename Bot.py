...
ITEMS = {
    "potion": {
        "price": 100,
        "desc": "Heal 50 HP saat battle"
    },
    "superpotion": {
        "price": 250,
        "desc": "Heal 100 HP saat battle"
    },
    "pokeball": {
        "price": 150,
        "desc": "Tambah +20% catch rate"
    },
    "buff_atk": {
        "price": 300,
        "desc": "+10 ATK sementara saat battle"
    }
}
...
def add_item(user_id, item, amount):
    ...
    """, (str(user_id), item, amount, amount))
...
def get_player_level(user_id):
    ...
    cursor.execute(
        "SELECT level FROM player WHERE user_id=?",
        (str(user_id),)
    )
...
def battle_engine(p1, p2, allow_heal=False):
    ...
    for turn in range(1, 21):  # max 20 turn
        ...
        # =========================
        if (
            attacker.get("user_id")
            and attacker["hp"] < attacker["max_hp"] * 0.5
            and use_item(attacker["user_id"], "potion")
        ):
            heal = 50
            attacker["hp"] = min(attacker["hp"] + heal, attacker["max_hp"])

            log.append(
                f"🧪 {attacker['name']} menggunakan Potion (+{heal} HP)"
            )
...
async def tutorial(ctx):
    await ctx.send("""(🎮 **TUTORIAL POKÉMON BOT (SINGKAT)**
...
🗺️ **Explore & Catch**
`!explore forest/ocean/cave/volcano`
`!catch`

⭐ Ada rarity: Common, Rare, Epic, Legendary
Semakin langka → semakin kuat

---

🎒 **Pokémon**
`!mypokemon` → lihat koleksi (ada gambar)
`!select <no>` → pilih aktif
`!feed <no>` → naik level
`!release <no>` → buang
...
🧪 **Item**
Potion → auto heal saat HP rendah
Pokéball → tambah chance tangkap
Buff ATK → tambah damage saat battle
...
💰 **Ekonomi**
`!money` → cek uang
`!sell <no>` → jual Pokémon

---

📈 **Level**
`!profile` → level & EXP
Naik level → unlock area

---

🔄 **Trade**
`!trade @user <punya> <dia>`
...
🔥 **Tips**
Gunakan Pokéball untuk Pokémon langka
Simpan Potion untuk PvP
Naikkan level Pokémon kamu
...
async def accept(ctx):
    ...
    # 💰 VALIDASI UANG
    # =========================
    if get_money(challenger.id) < bet:
        await ctx.send("❌ Uang challenger tidak cukup.")
        return
    ...
    # =========================
    # 🧠 BUILD PLAYER (ITEM READY)
    # =========================
    p1 = build_player(p1_data, challenger.id)
    p2 = build_player(p2_data, ctx.author.id)
    ...
    # 🏆 TENTUKAN PEMENANG
    # =========================
    if winner["user_id"] == challenger.id:
    ...
    # 💰 REWARD UANG
    # =========================
    update_money(winner_user.id, get_money(winner_user.id) + bet)
    update_money(loser_user.id, get_money(loser_user.id) - bet)

    # =========================
    # ⭐ EXP SYSTEM
    # =========================
    leveled, lvl, exp = add_exp(winner_user.id, 70)
    ...
    # batasi log biar tidak kepanjangan
    battle_text += "\n".join(log[:12])

    battle_text += f"\n\n🏆 **Pemenang: {winner_user.name}**"

    # =========================
    # 📤 KIRIM HASIL
    # =========================
    await ctx.send(battle_text)

    await ctx.send(
        f"💰 {winner_user.name} +{bet} coins\n"
        f"💸 {loser_user.name} -{bet} coins"
    )
    ...
    if leveled:
        await ctx.send(
            f"🆙 {winner_user.name} naik ke Level {lvl}!"
        )

def build_player(data, user_id):
    ...
    return {
        ...
        "type": ptype,
        "heal_used": False,
        "user_id": user_id   # 🔥 WAJIB untuk item system
    }
...
async def catch(ctx):
    ...
    if type1 in area_types or type2 in area_types:
        if use_item(user_id, "pokeball"):
            catch_rate += 20   # sesuai area → lebih mudah
    else:
    ...
    bonus = level // 10
    bonus += RARITY_BONUS.get(rarity, 0)
    bonus_hp = bonus
    bonus_atk = bonus
    bonus_def = bonus
    bonus_speed = bonus
...
async def mypokemon(ctx):
    ...
    for i, (name, level, bhp, batk, bdef, bspd, is_active) in enumerate(rows, start=1):
        ...
        embed = discord.Embed(
            title=f"{i}. {name} (Lv.{level}/{MAX_LEVEL}){active}",
            color=0x3B4CCA
        )
...
@bot.command()
async def explore(ctx, area: str = None):
    if area is None:
        await ctx.send(
            "❌ Gunakan `!explore <area>`\n"
            "Contoh:\n"
            "`!explore forest`\n"
            "`!explore ocean`\n"
            "`!explore cave`\n"
    ...
    # 🔒 Cek level unlock
    player_level = get_player_level(ctx.author.id)
    min_level = AREAS[area]["min_level"]

    if player_level < min_level:
        await ctx.send(
            f"🔒 Area {area.title()} terkunci!\n"
            f"🎯 Minimal Level: {min_level}\n"
            f"👤 Level kamu: {player_level}"
        )
        return

    # 🎯 Ambil pokemon berdasarkan area
    ...
    await ctx.send(
        ...
        f"✨ **{name} (Lv.{level}) [{rarity}] muncul!**\n"
        f"🧬 Type: {type_text}\n\n"
        f"Gunakan `!catch` untuk menangkap!"
    )

@bot.command()
async def areas(ctx):
    embed = discord.Embed(
        title="🗺️ Daftar Area Explore",
        description="Gunakan `!explore <area>` untuk menjelajah.",
        color=0x2ECC71
    )
    ...
    for area, types in AREAS.items():
        ...
        embed.add_field(
            name=f"🌍 {area.title()}",
            value=(
                f"🧬 Tipe dominan: {type_list}\n"
                f"🎯 Chance spawn tipe area: 70%\n"
                f"🎣 Catch rate tipe area: 85%"
            ),
            inline=False
        )
...
async def shop(ctx):
    ...
    for name, data in ITEMS.items():
        text += f"🔹 {name} - {data['price']} coins\n📄 {data['desc']}\n\n"

    text += "Gunakan: `!buy <item> <jumlah>`"
...
async def buy(ctx, item: str, amount: int = 1):
    ...
    if item not in ITEMS:
        await ctx.send("❌ Item tidak tersedia.")
        return

    price = ITEMS[item]["price"] * amount
    money = get_money(ctx.author.id)

    if money < price:
        await ctx.send("❌ Uang tidak cukup.")
        return

    update_money(ctx.author.id, money - price)
    add_item(ctx.author.id, item, amount)

    await ctx.send(f"🛍️ Berhasil beli {amount}x {item}!")

@bot.command()
async def bag(ctx):
    items = get_items(ctx.author.id)

    if not items:
        await ctx.send("🎒 Inventory kosong.")
        return
...
async def use(ctx, item: str):
    ...
    if not use_item(ctx.author.id, item):
        await ctx.send("❌ Item tidak ada.")
        return
    
    if item == "buff_atk":
        # contoh efek sederhana
        await ctx.send("⚔️ ATK boost aktif di battle berikutnya!")
    else:
        await ctx.send(f"✅ Kamu menggunakan {item}!")
